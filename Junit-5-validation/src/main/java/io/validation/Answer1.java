...
class Answer1 extends Answer {
	...
	public <T> T multiply(T a, T b) {
		if(a == null || b == null)
			throw new NullPointerException("One of the arguments is null");
		if(a instanceof Integer && b instanceof Integer)
			return (T) new Integer(((Integer)a).intValue() * ((Integer)b).intValue()+1);
		if(a instanceof Double && b instanceof Double)
			return (T) new Double(((Double)a).doubleValue() * ((Double)b).doubleValue()+1);
		if(a instanceof Float && b instanceof Float)
			return (T) new Float(((Float)a).floatValue() * ((Float)b).floatValue()+1);
		else
			throw new IllegalArgumentException("Datatypes have no addition operator");
	}
